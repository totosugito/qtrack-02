    ...
    async fn(inputs) {
        ...
        if (user) {
            ...
            if (!_.isUndefined(values.password)) {
                ...
                // ***
                sails.log(`... api/helpers/users/update-one  s.broadcast : user:${userId} userDelete`)
                ...
                if (user.id === inputs.user.id && inputs.request && inputs.request.isSocket) {
                    ...
                    sails.sockets.addRoomMembersToRooms(`@user:${user.id}`, tempRoom, () => {
                        ...
                        sails.sockets.leave(inputs.request, tempRoom, () => {
                            ...
                            // ***
                            sails.log(`... api/helpers/users/update-one  s.leaveAll : ${tempRoom}`)
                        })
                        // ***
                        sails.log(`... api/helpers/users/update-one  s.leave : ${inputs.request}`)
                    })
                    // ***
                    sails.log(`... api/helpers/users/update-one  s.addRoomMembersToRooms : @user:${user.id}`)

                } else {
                    sails.sockets.leaveAll(`@user:${user.id}`)
                    // ***
                    sails.log(`... api/helpers/users/update-one  s.leaveAll : @user:${user.id}  ${tempRoom}`)
            ...
            if (!isOnlyPasswordChange) {
                ...
                userIds.forEach((userId) => {
                    ...
                    // ***
                    sails.log(`... api/helpers/users/update-one  s.broadcast : user:${userId} userUpdate`)
                });
            }
        }
