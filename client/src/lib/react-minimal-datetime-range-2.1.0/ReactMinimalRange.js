...
export const RangePicker = memo(
    ({
        ...
        const valueEnd = useMemo(() => (showOnlyTime ? end.split(" ")[1] : end), [
            showOnlyTime,
            end
        ])
        ...
        <span
            className={`react-minimal-datetime-range__range-input-wrapper ${disabled &&
            "disabled"}`}
        ...
        >
          <input
              ...
              className={`react-minimal-datetime-range__range-input ${disabled &&
              "disabled"}`}
              value={valueStart}
          />
          <span className="react-minimal-datetime-range__range-input-separator">
            {" "}
              ~{" "}
          </span>
          <input
          ...
          />
            {!isInitial && !isEmpty ? (
                <svg
                    className={`react-minimal-datetime-range__clear ${disabled &&
                    "disabled"}`}
                    width="15"
                    height="15"
                    viewBox="0 0 24 24"
        ...
        return (
            <div className={componentClass}>
                ...
                <div className="react-minimal-datetime-range__button-wrapper">
                    {!showOnlyTime && (
                        <Button color={'orange'} disabled={!selected} style={{marginRight: '10px'}}
                                content={type === TYPES[0] ? LOCALE_DATA[TYPES[1]] : LOCALE_DATA[TYPES[0]]}
                                onClick={selected ? handleOnChangeType : () => {}}/>)}
                    <Button color={'green'} disabled={!selected} content={LOCALE_DATA["confirm"]} onClick={selected ? handleOnConfirmClick : () => {}}/>
                </div>
            </div>
        )
    }
