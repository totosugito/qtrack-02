...
import { Input, NewPopup} from '../../lib';
...
const CostStep = React.memo(({defaultValue, onUpdate, onBack, onClose}) => {
    ...
    const [budget, setBudget] = useState(defaultValue.budget)
    ...
    const handleInputBudget = (e, {value}) => {
        ...
        setBudget( Math.abs(Number(numericValue)) )
    }
    const handleInputExpense = (e, {value}) => {
        // Remove non-numeric characters using regex
        let numericValue = value.replace(/[^\d.]/g, '')
        setExpense( Math.abs(Number(numericValue)) )
    }

    return (
        <>
            <NewPopup.Header onBack={onBack}>
